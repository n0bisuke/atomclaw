...
Telegramの代わりにDiscordのスラッシュコマンドで会話し、オプションでCloudflare KVに会話履歴を保存します。
...
5. [PlatformIO でビルド・書き込み](#5-platformio-でビルド書き込み)
...
初回ビルドはESP-IDFのダウンロードがあるため10〜20分かかります。
...
> **重要:** ESP32-S3ボードには通常USBポートが2つあります。
> **「USB」または「UART」と書かれたポート**（シリアル/JTAGポート）を使用してください。
