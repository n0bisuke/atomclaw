...
esp_err_t http_proxy_init(void)
{
    ...
    if (CFG_SECRET_PROXY_HOST[0] != '\0' && CFG_SECRET_PROXY_PORT[0] != '\0') {
        strncpy(s_proxy_host, CFG_SECRET_PROXY_HOST, sizeof(s_proxy_host) - 1);
        s_proxy_port = (uint16_t)atoi(CFG_SECRET_PROXY_PORT);
    }

    /* NVS overrides take highest priority (set via CLI) */
    nvs_handle_t nvs;
    if (nvs_open(CFG_NVS_PROXY, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[64] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, CFG_NVS_KEY_PROXY_HOST, tmp, &len) == ESP_OK && tmp[0]) {
            strncpy(s_proxy_host, tmp, sizeof(s_proxy_host) - 1);
            uint16_t port = 0;
            if (nvs_get_u16(nvs, CFG_NVS_KEY_PROXY_PORT, &port) == ESP_OK && port) {
                s_proxy_port = port;
            }
        }
        nvs_close(nvs);
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(CFG_NVS_PROXY, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, CFG_NVS_KEY_PROXY_HOST, host));
    ESP_ERROR_CHECK(nvs_set_u16(nvs, CFG_NVS_KEY_PROXY_PORT, port));
...
esp_err_t http_proxy_clear(void)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(CFG_NVS_PROXY, NVS_READWRITE, &nvs));
    nvs_erase_key(nvs, CFG_NVS_KEY_PROXY_HOST);
    nvs_erase_key(nvs, CFG_NVS_KEY_PROXY_PORT);
    nvs_commit(nvs);
    nvs_close(nvs);
